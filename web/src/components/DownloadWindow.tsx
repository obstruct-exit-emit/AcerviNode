...
import { writeFileToDirectory } from '../fsAccess'
import type { AddBatchMessage, BatchCompleteMessage, BatchProgressMessage } from '../downloadWindowProtocol'
...
export function DownloadWindow() {
  ...
  useEffect(() => {
    ...
    function onMessage(e: MessageEvent<AddBatchMessage>) {
  ...
  async function processBatch(batch: AddBatchMessage, totalBytes: number) {
    ...
    if (!apiKey) {
      setBatches((prev) => ({ ...prev, [batch.downloadId]: { ...prev[batch.downloadId], status: 'error' } }))
      return
    }
  ...
  function reportComplete(downloadId: string, failed: string[]) {
