...
`cleanup_after_days`, `download_dir_mode`, and `fast_poll_interval_seconds`
can all be changed together in one call via `PUT /api/v1/settings/general`
(alongside `port`/`data_dir`/`tls_enabled`/`tls_port`, which persist but
need a restart) — see [API](api.md).
