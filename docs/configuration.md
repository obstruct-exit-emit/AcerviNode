...
| `backup_interval_hours` | `ACERVINODE_BACKUP_INTERVAL_HOURS` | `24` | How often AcerviNode snapshots its own database to `backup_dir`. The one retention setting here that defaults to **on**, and deliberately so: every other `0`-default above *removes* something, so doing nothing is the safe default, whereas doing nothing here is the only case that has a cost — losing the database loses the configuration, the download history, the categories and every login account at once. `0` disables scheduled backups; manual ones still work. Not range-validated — a negative value simply disables it the same way `0` does. No backup is taken at startup, so a restart loop can't flush the useful older snapshots out of the retention window. Editable live (no restart) — a changed interval retunes the schedule immediately rather than waiting out the old one |
...
`backup_keep`, `managed_add_delete_after_fetch` and `managed_add_keep_files`
can all be changed
