...
| `auth.users` | *(none — set via API/UI, not env)* | *(none — login disabled)* | Optional login accounts, on top of the API key (which keeps working unaffected). No accounts means login is disabled entirely. Managed via the web UI's Settings → Security, the first-run setup wizard, or `/api/v1/settings/users`/`/api/v1/setup` directly — see [Providers](providers.md#auth-login-accounts-and-roles) for the full design (roles, the protected Default account, session mechanics). Never hand-edit a password hash into this file — there's no supported way to generate one outside the app |
