...
| `backup_interval_hours` | `ACERVINODE_BACKUP_INTERVAL_HOURS` | `0` (disabled) | How often AcerviNode snapshots itself to `backup_dir`. **Opt-in** — set a value to enable it. Each snapshot is **two files** sharing one timestamp: `acervinode-<stamp>.db`, the database, and `acervinode-<stamp>.yaml`, a copy of the config file. That split matters, because the two hold different things — the database has the download history and discovery state, while the config has every setting, both provider API keys, the AcerviNode API key and every login account. A snapshot of only the database would restore your history and leave you locked out. Off by default because the config half is a plaintext copy of your credentials, and writing one on a schedule is a decision to make deliberately rather than inherit; both files are written `0600`, readable only by the user AcerviNode runs as. `0` disables scheduled backups; manual ones still work. Not range-validated — a negative value simply disables it the same way `0` does. The schedule is timed from the newest snapshot on disk, not from when the process started, so an instance restarted more often than its interval still backs up — while a restart loop still cannot produce more than one snapshot per interval, since each restart sees the one before it. Editable live (no restart) — a changed interval retunes the schedule immediately rather than waiting out the old one |
