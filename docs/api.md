...
| `GET` | `/api/v1/downloads/torrent/info` | Metadata preview for a magnet or hash without adding it — name, size, file list. Asks the **default** provider and never quietly a different one: a lookup discloses the hash to whichever service answers it, and doing that behind the caller's back would leak a hash aimed at one provider to another. A provider that has no preview feature says so; pass `?provider=<name>` to ask a specific one, which makes it a decision rather than a side effect. See [Cached & metadata previews](#cached--metadata-previews) |
