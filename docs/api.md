...
(see `GET /api/v1/settings/providers`, which never echoes a provider's
actual key back), `GET /api/v1/settings/general` does return AcerviNode's own
...
backups, user management) additionally requires the **admin** role — a
`member` login account gets `403`. There is exactly one exception:
`PUT /api/v1/settings/users/{username}/password`, so that any signed-in
account can change *its own* password without an admin doing it for them
(changing someone else's still requires admin). A member's access is
otherwise scoped to Manual downloads only; see the same Providers section
...
| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/v1/health` | Unauthenticated liveness check — `{"status": "ok"}`. Answers as long as the HTTP server is up; it says nothing about whether polling is working, which is what `/api/v1/status` below is for |
| `GET` | `/api/v1/version` | `{"version": "..."}` — the build's version string, stamped at release time via `-ldflags "-X main.version=..."`. A build made without that (a plain `go build`) reports `0.0.0-dev` |
...
| `POST` | `/api/v1/downloads/{id}/retry` | Re-runs the **local fetch** for a download, resetting it to `provider_completed`. Valid for `error`, `ready_for_import` and `provider_completed`; refused with `409` for `queued`/`downloading`, since forcing a row the provider hasn't finished into `provider_completed` would have the importer fetch something that doesn't exist yet. Deliberately wider than `error` alone: a download can be wrong without having given up — a `ready_for_import` row whose files never landed being the case that motivated it — and delete-and-re-add is a poor only-answer to that|
...
| `GET` | `/api/v1/downloads/{torrent,usenet,webdl}/check-cached` | Asks the provider whether something is already cached **before** adding it — `?magnet=`/`?hash=` for torrent, `?url=` for usenet, `?link=` for webdl. See [Cached & metadata previews](#cached--metadata-previews) |
| `GET` | `/api/v1/downloads/torrent/info` | Metadata preview for a magnet or hash without adding it — name, size, file list. Asks the **default** provider, and only that one. A lookup discloses the hash to whichever service answers it, so crossing providers would leak a hash aimed at one provider to another — and unlike the add fallbacks, which prevent a download failing and surface visibly under whoever accepted it, this only saves a preview panel. A provider with no preview feature says so plainly, and there is deliberately no way to name a different one: a preview a provider can't give is a preview you don't get. See [Cached & metadata previews](#cached--metadata-previews) |
| `GET` | `/api/v1/settings/providers` | Every *registered* provider: `[{"name", "type", "configured", "torrent_capable", "usenet_capable", "webdl_capable", "torrent_enabled", "usenet_enabled", "webdl_enabled", "default"}]` — never the actual key, only whether one is set. `*_capable` is what the provider's **service** can do at all; `*_enabled` is whether it is switched on here (always `false` when the matching `*_capable` is). The two are separate so a client can tell "this service has no usenet" from "usenet is turned off" — see `PUT .../kinds` above. `type` is which implementation the entry uses, equal to `name` for a first account and different when one service holds several, which is what lets the UI offer "another TorBox account" from TorBox's own card without asking which service it is. Includes providers holding no credentials yet, since this is the surface you configure them from; contrast `GET /api/v1/providers`, which answers "what can I use right now" and omits them |
| `PUT` | `/api/v1/settings/providers/{name}` | Body `{"api_key": "..."}` — applies a key to that provider immediately (no restart) and persists it. An **empty** `api_key` clears the provider's credentials rather than being rejected: that's how a provider is switched off without hand-editing `config.yaml`, and it stays registered so it can be configured again later. Clearing a key empties it **in place** rather than removing the entry, so everything else the entry holds — its `type`, and any `disabled_kinds` — survives, including across a restart. Deleting an entry outright is what `DELETE .../providers/{name}` is for. `404` for a provider this build doesn't know about, so a typo is visible instead of looking like it worked |
| `POST` | `/api/v1/settings/providers` | Body `{"name": "...", "type": "...", "api_key": "..."}` — registers a new provider entry live and persists it, no restart. `api_key` is optional (adding the entry and pasting its key can reasonably be two steps). An omitted `type` means "same as the name". The name/type split is what allows **two accounts on one service**: entries `torbox` and `torbox-work` both of type `torbox` are independent providers, each with its own credentials, listing cache and rate-limit backoff. `400` for an empty name or an unknown type, `409` if that name already exists. `201` on success |
| `POST` | `/api/v1/settings/providers/{name}/reset` | Returns one provider to how it looks before anything is configured: credentials cleared, every kind its service supports switched back on. The **entry is kept**, so the provider stays listed and can be set up again — including a second account's `type`, which lives nowhere else. Deliberately leaves `default_provider` alone: which provider new downloads go to is a decision about the instance, not a property of the provider being tidied. `204` on success.

Use this rather than `DELETE` for a provider this build knows about, where deleting is a slight lie — the entry goes, but the provider is rebuilt from the known list on the next start and comes back unconfigured. `DELETE` remains the honest answer for a second account, which really can be removed because nothing recreates it. The web UI offers exactly that split |
| `DELETE` | `/api/v1/settings/providers/{name}` | Removes a provider entry and its credentials, live. Downloads already tracked against it are deliberately **left alone** — they are records of real things, and removing a provider is a configuration change, not an instruction to discard history; they simply stop resolving to a provider. If the removed entry was the default, the default moves to whatever is registered first, so an add immediately afterwards still resolves somewhere. `404` for an unknown name, `204` on success.

Removing a provider *type* from the build is a different thing again, and is covered in [Providers](providers.md#removing-a-provider) — downloads tracked against it stay listed and deletable, but their provider-side copies can no longer be reached. For a provider this build knows about, prefer `POST .../reset` above: deleting one only clears it until the next start, when it is rebuilt from the known list and reappears unconfigured. Deleting is the right verb for a **second account**, which exists solely in its config entry and really is gone — the web UI offers exactly that split, and labels the second-account button "Remove account" |
| `GET` | `/api/v1/settings/provider-types` | `["torbox", "alldebrid"]` — the provider implementations this build can construct, for the "add an account" flow. A provider's *name* is free text; its **type** is not, and must come from this list |
| `GET` | `/api/v1/settings/providers/{name}/account` | That one provider's own account status — same shape as `/api/v1/settings/account` below. Per provider rather than one call for the instance, because each account has its own plan and its own restrictions: showing one provider's cooldown under a heading that could mean either would be worse than showing nothing. Always `200` with `available: false` on failure — a provider being unreachable is a state to display, not an error to handle. `404` only for a name that isn't registered at all |
...
| `DELETE` | `/api/v1/settings/backups/{name}` | Removes one snapshot **and the config copy beside it** — the pair goes together, exactly as pruning removes it. `204` on success, `404` if there is no such snapshot, and `400` for a name this instance never wrote. That last one is not a formality: the name arrives as a URL path segment, so it is validated against the exact `acervinode-<stamp>.db` shape — the timestamp has to parse — and rejected before anything touches the filesystem. Deleting is offered where restoring is not, and the asymmetry is deliberate: removing a snapshot from a browser is tidying, putting one back is a decision that deserves a shell. Admin only |
| `POST` | `/api/v1/settings/backups/{name}/restore` | Puts a snapshot back. Returns **202**, not 200: SQLite will not have its file swapped under an open connection, so the snapshot is staged and AcerviNode restarts to apply it before anything opens the database. The client sees the response, then the connection drop. The database it replaced is kept as `acervinode.db.pre-restore` rather than discarded. **Only the database half is restored** — the config half holds the API key and every login, so replacing it would sign out the session that asked and invalidate the credential the request authenticated with; that stays a shell job, and the snapshot's `.yaml` is on disk for it. Same name validation as `DELETE`. Admin only |
| `GET` | `/api/v1/settings/account` | The **default** provider's own account status — with several configured, use `/api/v1/settings/providers/{name}/account` above to ask about a specific one. Kept because it predates multi-provider support and still answers the common single-provider case (plan tier, subscription state, premium expiry, lifetime bytes downloaded) — a live call, not a cached snapshot. Always HTTP 200: `{"available": false, "error": "..."}` if nothing's configured yet or the provider doesn't support this; `{"available": true, "plan_name": "...", "is_subscribed": bool, "premium_expires_at": "...", "total_bytes_downloaded": N, "cooldown_until"?: "..."}` otherwise. `cooldown_until` is TorBox-specific, undocumented by TorBox itself, and only present when set — see [Providers](providers.md#cooldown_until--a-real-undocumented-account-restriction) for what it means and how it was found. See [Providers](providers.md#accountprovider) |
...
| `GET` | `/api/v1/setup/status` | Unauthenticated. `{"needed": bool}` — whether this instance is claimable by its first visitor — simply, whether no login account exists yet. A configured provider does **not** close the window: login is mandatory and there's no API-key-only way into the web UI, so an instance with a provider key but no account is still a fresh install rather than an upgrade case to special-case around. See [Providers](providers.md#auth-login-accounts-and-roles)) |
