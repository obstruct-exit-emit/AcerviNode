...
> builds fine here. Production deployment targets Linux (same posture as
> LibriNode) — the local-mount phase in particular (see the
> [roadmap](../ROADMAP.md)) will need a Linux host.

## Layout

```
cmd/acervinode/         entrypoint: config, database, provider registry, HTTP servers
